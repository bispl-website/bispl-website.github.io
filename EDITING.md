...
- **이미지 교체**: 새 그림을 `assets/img/highlight/` 폴더에 넣고, 위 `image:`를 그 파일명으로.
  - ⚠️ **파일명이 한 글자라도 다르면 그림이 안 나옵니다** (빌드는 성공하고 그림 자리만 비어 보입니다). 업로드한 실제 파일명을 그대로 복사해 넣으세요. 대소문자와 확장자(`.png`/`.jpg`)도 구분합니다.
  - ⚠️ `url:` 에는 `https://` 를 **한 번만** 씁니다. 주소를 붙여넣을 때 `https://https://...` 가 되기 쉬운데, 이러면 링크가 깨집니다.
...
- 항목 2
```
- ⚠️ **파일명은 반드시 `.md` 로 끝나야 합니다.** 확장자를 빠뜨리면 마크다운 변환이 일어나지 않아 문단·목록이 전부 한 줄로 붙어 나옵니다 (빌드는 성공합니다).
- ⚠️ **목록은 하이픈 하나 + 공백** (`- 항목`) 입니다. `--` 로 쓰면 목록이 아니라 그냥 글자로 나옵니다.
